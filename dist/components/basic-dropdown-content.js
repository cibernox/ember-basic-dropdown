...
class BasicDropdownContent extends Component {
  ...
  respondToEvents = modifier(dropdownElement => {
    this.args.dropdown?.actions?.registerDropdownElement && this.args.dropdown.actions.registerDropdownElement(dropdownElement);
    ...
    this.handleRootMouseDown = e => {
      ...
      this.args.dropdown?.actions?.close && this.args.dropdown.actions.close(e, true);
    };
...
function dropdownIsValidParent(triggerElement, el, dropdownId) {
  ...
  if (closestDropdown === null) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const closestAttrs = closestDropdown.attributes;
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const parentAttrs = parentDropdown.attributes;
    return parentDropdown && parentAttrs.id.value === dropdownId || dropdownIsValidParent(triggerElement, parentDropdown, dropdownId);
  }
}
