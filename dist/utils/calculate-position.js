...
const getViewData = (trigger, content, renderInPlace) => {
  let scroll = {
...
function calculateInPlacePosition(trigger, content, _destination, {
  horizontalPosition,
  verticalPosition
}) {
  let dropdownRect;
  ...
  if (verticalPosition === 'above') {
    ...
    const enoughRoomBelow = triggerTop + triggerHeight + dropdownHeight < viewportBottom;
    const enoughRoomAbove = triggerTop > dropdownHeight;
    if (enoughRoomBelow) {
      verticalPosition = 'below';
...
function calculatePosition(trigger, content, destination, options) {
