...
<template>
  <h1 class="doc-page-title">Overlays</h1>
  ...
  <CodeExample @glimmerTs="overlays-1.gts">
    <Overlays1Component />
  </CodeExample>
  ...
  <ul>
    <li>
      ...
      <code>$ember-basic-dropdown-overlay-background</code>
      SASS variable or target the
      <code>.ember-basic-dropdown-overlay</code>
    </li>
    <li>
      By default overlays have
      <code>pointer-events: none</code>, so it is transparent to clicks but you
      can change that using the
      <code>$ember-basic-dropdown-overlay-pointer-events</code>
